...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.loading = true
      if (this.eidt) {
        this._updateCategory({ name: typeName, parentid: id, id: this.childId, icon: uploads.url, type: 2 })
      } else {
        this._saveCategory({ name: typeName, parentid: id, icon: uploads.url, type: 2 })
      }
    },
    _updateCategory(e) {
