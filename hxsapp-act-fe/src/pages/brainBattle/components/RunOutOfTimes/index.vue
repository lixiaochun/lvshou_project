<template>
  <div class="NumberOfGames">
    <div v-bind:class="{contentBox: true,openBox: openBox,closebox: closebox}">
      ...
      <div class="closeImageBox" @click="BoxClose">
        <img :src="closeImage" alt="" class="closeImage"/>
      </div>
      <div class="content">
         <div class="textContent">
           <div class="rtitle">让脑袋休息一会</div>
           <div class="rtcon">明儿再战吧</div>
         </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  props: [ 'selfNumber', 'inviteFriendsNumber', 'releaseDynamicsNumber' ],
  mounted() {
  ...
  methods: {
    BoxClose() {
      if (this.$parent.BoxClose && typeof this.$parent.BoxClose === 'function') {
        ...
        this.closebox = true;
        setTimeout(function () {
          this.$parent.RunOutOfTimesShow = false
        }.bind(this), 400)
      }
    }
  }
