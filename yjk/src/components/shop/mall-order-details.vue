          ...
          <div class="trading">
            <el-table
              ...
              <el-table-column label="价格" width="200" align="center">
                <template slot-scope="scope">
                  <div>
                    <span v-if="tableData[scope.$index].price > 0">￥{{_formatCurrency(tableData[scope.$index].price)}}元</span>
                    <span v-if="tableData[scope.$index].price > 0 && tableData[scope.$index].points > 0">+</span>
                    <span v-if="tableData[scope.$index].points > 0">{{tableData[scope.$index].points}}魅力分</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="购买方式" width="200" align="center">
                <template slot-scope="scope">
                  <div>{{tableData[scope.$index].buyStyle}}</div>
                </template>
              </el-table-column>
            </el-table>
            <div class="combined-content" v-if="tableData && tableData.length > 0">
              <ul class="combined">
                <li><div>合计：</div></li>
                <li><div style="text-align: center; padding: 0 10px; text-indent: 0px;">x{{countNumber}}</div></li>
                <li>
                  <div style="text-align: center; padding: 0 10px; text-indent: 0px;">
                    <span v-if="countAmount > 0"> ￥{{_formatCurrency(countAmount)}}元</span>
                    <span v-if="countAmount > 0 && countPoints > 0"> + </span>
                    <span v-if="countPoints > 0"> {{countPoints}}魅力分</span>
                  </div>
                </li>
                <li></li>
              </ul>
              ...
              <ul class="realPay">
                <li><div>实付：</div></li>
                <li><div>现金：￥{{_formatCurrency(finallPrice)}}</div></li>
                <li><div>魅力分：{{allPoints}}</div></li>
              </ul>
            </div>
          </div>
        </div>
...
import { getOrderdetail, getGoodsByOrder, getOrderLogistics, subOrderRemark } from 'api/mall'
...
export default {
  ...
  methods: {
    ...
    _getOrderdetail(id) {
      ...
      getGoodsByOrder({ orderId: id }).then(res => {
        if (res.code === ERR_SUCCESS) {
          const tableData = res.data.goodsList
          const data = res.data
          this.tableData = tableData
          this.vipDescr = data.vipDescr
          this.allPoints = res.data.spendPoints
          this.finallPrice = res.data.payableAmount
          this.countPoints = data.countPoints
          this.countAmount = data.countAmount
          this.countNumber = data.countNumber
    ...
    //     this.allPrice += price
    //     const points = parseInt(e['points']) * parseInt(e['qty'])
