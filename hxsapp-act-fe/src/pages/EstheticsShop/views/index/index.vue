<template>
  <div class="wrapper">
    ...
    <div class="img-box">
      <img src="../../asstes/image/1.png" @click="popupType=false">
      <img src="../../asstes/image/2.png"  @click="popupType=false" >
      <div class="row">
        <div class="popup" v-if="popupType" :style="{'bottom':popupData.bottom}">
          <p class="big">{{popupData.name}}</p>
          <p class="small">{{popupData.content}}</p>
          <div class="angle" :style="{'right':popupData.right}">
                <!-- <span class="l"></span>
                <span class="r"></span> -->
                <i class="el-icon-arrow-down" ></i>
           </div>
        </div>
        <img v-for="(item, index) in imgLists1"  @click="getPopup(index)"  :src="item.src">
      </div>
      <img v-for="(item, index) in imgLists2"  :src="item.src"  >
       <img   src="../../asstes/image/11.png"   v-if="type.coupon!=1"  @click="getCoupon('coupon')"  >
       <img   src="../../asstes/image/21.png"   v-if="type.coupon==1"  >
       <img v-for="(item, index) in imgLists3"  :src="item.src"  >
      <div class="from-box"  @click="popupType=false">
          <el-form  label-width="80px" :model="fromdata" ref="fromdata">
              <el-form-item label="姓名" prop="name" :rules="{
                required: true, message: '姓名不能为空', trigger: 'blur'
              }">
                <el-input v-model="fromdata.name" :maxlength="6"
              ...
              <el-form-item label="手机号码" prop="num" :rules="{
                required: true, message: '手机号码不能为空', trigger: 'blur'
              }">
                <el-input v-model="fromdata.num" :maxlength="11"
              ...
              { type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change'] }
                ...
                <el-input v-model="fromdata.email" :maxlength="50"    :disabled="type.disabled" ></el-input>
              </el-form-item>
              <el-form-item label="留言">
                <el-input type="textarea" v-model="fromdata.content" :rows="4" :maxlength="100"    :disabled="type.disabled"></el-input>
              </el-form-item>
            
            </el-form>
      </div>
  
      <img src="../../asstes/image/18.png"  @click='getData(false)'  v-if="type.commit!=1">
      <img src="../../asstes/image/22.png"  @click='getData(true)'  v-if="type.commit==1">
      <img src="../../asstes/image/19.png" :style="{'marrgin-bottom':'0.5rem'}" @click="popupVisible=false">

    </div>
    <a href="tel://4008880887" class="tocell"  @click="popupType=false">
      <img src="../../asstes/image/20.png">
    </a>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted: function () {
      var that=this;
      axios.get('https://act.hxsapp.com/art/Apply/checkstatus', {
              params: {
                ...
                if(parseInt(res.data.code)==200){
                   that.type.coupon=parseInt( res.data.data.coupon.status);
                   that.type.commit=parseInt( res.data.data.commit.status);
                   if(  that.type.coupon==1){
                    
                    }
                    if(that.type.commit==1){
