...
.cov-vue-date .datepickbox .left,
.cov-vue-date .datepickbox .time_text,
.cov-vue-date .datepickbox .right{
  color: #4a4a4a;
  font-size: 16px;
  background: #F5F5F8;
  display: inline-block;  
...
<template>
  <div class="cov-vue-date" :class="option.wrapperClass ? option.wrapperClass : {}">
    <div class="datepickbox">
      ...
      <input type="text" v-if="0 && showInfo.check"
        title="input date" 
        class="cov-datepicker" 
        readonly="readonly" :placeholder="option.placeholder" 
      ...
      <span class="time_text" @click="showCheck">
        {{date.time}}
      </span>
        ...
        <div class="cov-date-box" v-if="showInfo.day">
          <div class="cov-picker-box">
            ...
            <div class="day" v-for="day,index in dayList" :key="index" @click="checkDay(day)" 
              ...
              :style="day.checked ? (option.color && option.color.checkedDay ? { background: option.color.checkedDay } : { background: 'rgba(255,104,176,.9)' }) : {}">
              {{day.value}}
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    checkDay (obj) {
      ...
      switch (this.option.type) {
        case 'day':
          // this.picked()
          console.log("// this.picked()这里不调用了")
          break
        case 'min':
          this.showOne('hour')
    ...
    picked () {
      ...
      if( this.limit[0] && this.limit[0].from && this.limit[0].to ){
        var isTo = moment( this.date.time ) <= moment( this.limit[0].to );
        var isFrom = moment( this.date.time ) >= moment( this.limit[0].from );

        if( !( isFrom && isTo ) ){
          Toast("请选择正确的起止日期！")
          this.date.time = this.checked.oldtime;
          return;
        }else{
