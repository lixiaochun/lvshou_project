		...
		<div class="discuss-container">
			...
			<div class="discuss-main clearfix">
				...
				<div class="textarea-box">
					...
					<!-- 同时分享到动态按钮的事件 -->
					<div class="share-to-dyanmic" @touchstart.stop="shareToDyanmicFn"  v-if="discussCommendNum">
...
export default {
	...
	mounted() {
		...
		//用户完成每日任务隐藏栏绑定事件
		document.body.addEventListener('touchmove',_this.hxsapp_accomplishTasksTipNone,false);
		//右边3个按钮样式
	...
	methods: {
				...
				if (this.isLogin) {
					...
					if( this.soucrePage == "sportsLessonDetail"){
						...
						__json = {
							...
							data_id: __data_id,
							sess_token: commonJs.getUrlParam("sess_token"),
							links: commonJs.config.proUrl +'hxsweb/sportsLessonDetail/' + this.scheme_id + '/' + this.plan_id + '/' + this.course_id,
							content : { 
			...
			if (this.isApp) {
				...
				if(this.soucrePage == "videoSpecial"){//运动课程
					...
					commonJs.buriedPoint({
						...
						access_occurred_type_id: _this.course_id,//发生访问的【页面】类型ID
						previous_event_type_obj:'' ,//【事件】对象类型
						previous_event_type_obj_id: '',//【事件】对象类型ID
						previous_content_sort: ''//【事件】事件参数
					});
		...
		speak() {
			...
			//发表评论
			axios.get('https://api.hxsapp.com/community/userComment/doComment', {
				params: {
				...
				if (res.data.code == 200) {
					...
					if( this.$route.soucrePage && this.$route.soucrePage != ""){
						...
						}else if( this.$route.soucrePage == "sportsLessonDetail"){
					...
					if( insert_id ) {
						axios.get('https://api.hxsapp.com/community/userComment/getCommentById', {
							params: {
					...
					if (userInfo) {
						...
						if (this.shareToDyanmic) {
							axios.get('https://api.hxsapp.com/community/userDiary/saveDiary', {
								params: {
									content: this.message,
									type: 2,
		...
		iosFixed() {
			...
			Vue.nextTick( () => {
				...
				commonJs.iosInputfixed('.discuss-container', '.fixed_input', function() {
					//focus操作
					...
					if (!_this.commentInfo.isArguedName) {
						$('.textarea-box textarea').attr('placeholder', '来发表你的高见吧');
					} else {
						$('.textarea-box textarea').attr('placeholder', '回复' + _this.commentInfo.arguedName);
					...
					if( _this.$route.soucrePage && _this.$route.soucrePage != ""){
						...
						// 组件调用类型埋点
						if( _this.soucrePage == "sportsLessonDetail"){
							//埋点
							commonJs.buriedPoint({
		...
		discussCommendNumFn(){
			if(window.location.href.indexOf('commentChild')==-1){this.discussCommendNum = true;}
		},
		hxsapp_accomplishTasksTipNone:function(){//隐藏用户完成每日任务底部提示行为
		...
		monitorTextareaValue(){
			this.monitorTextareaValueTime=setInterval(()=>{
				...
				if(value + '' && !value.match(/^\s+$/)){
					$('.textarea-box_active .top a').eq(1).removeClass('notAllow');
				}else{
					$('.textarea-box_active .top a').eq(1).addClass('notAllow');
				}
			},10);
		},
		seriesFnDivWidth(){
	...
	beforeDestroy() {
		...
		// debugger
		//解除用户完成每日任务隐藏栏事件
		document.body.removeEventListener('touchmove',_this.hxsapp_accomplishTasksTipNone,false);
	},
	components:{
		loginFrame
	}
